"""
MSSQL Connector - Real implementation using SQLAlchemy
"""
import asyncio
import urllib.parse
...
class MSSQLConnector(BaseConnector):
    """
    MSSQL Connector using SQLAlchemy with pymssql driver.
    ...
    def _build_connection_url(self) -> str:
        ...
        # Use pymssql driver
        connection_url = f"mssql+pymssql://{username}:{encoded_password}@{host}:{port}/{database}"
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            add_log(f"MSSQLConnector: Successfully connected to {self._credentials.get('host')}")
            return True
        except Exception as e:
            self._connected = False
            add_log(f"MSSQLConnector: Connection failed: {str(e)} | traceback: {traceback.format_exc()}")
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            add_log(f"MSSQLConnector: Connection test failed: {str(e)}")
            self._connected = False
            return False
    ...
    async def get_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            add_log(f"MSSQLConnector: Retrieved schema - {len(tables)} tables, {len(views)} views")
            return {
                "tables": tables,
                "views": views
            }
        except Exception as e:
            add_log(f"MSSQLConnector: Schema retrieval failed: {str(e)} | traceback: {traceback.format_exc()}")
    ...
    async def get_data(
        ...
        **kwargs
        ...
        try:
            ...
            add_log(f"MSSQLConnector: Fetched {len(df)} rows from {table_name}")
            return df
        except Exception as e:
            add_log(f"MSSQLConnector: Data fetch failed for {table_name}: {str(e)} | traceback: {traceback.format_exc()}")
    ...
    async def disconnect(self) -> None:
        ...
        try:
            if self._engine:
                ...
                self._engine = None
            self._connected = False
            add_log("MSSQLConnector: Disconnected")
        except Exception as e:
            add_log(f"MSSQLConnector: Error during disconnect: {str(e)}")
